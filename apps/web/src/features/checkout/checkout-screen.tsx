...
export function CheckoutScreen({
  ...
  // Fail closed while CheckoutPayment validates any session-only attempt. A
  // draft change alters the server replay fingerprint, so it must not race a
  // pending or unknown payment attempt.
  const [paymentAttemptUnsettled, setPaymentAttemptUnsettled] = useState(true);
  ...
  useEffect(() => {
    ...
    void transport.loadDraft().then(
      (draft) => {
        if (draft) clearCheckoutHandoff(window.sessionStorage);
        if (!active) return;
        if (draft) {
  ...
  const setValue = (name: keyof CheckoutForm, value: string) => {
    if (paymentAttemptUnsettled) return;
  ...
  const save = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (paymentAttemptUnsettled || saveState === 'saving') return;
    setSaveState('saving');
    try {
  ...
  const preserveForAuth = (event: React.MouseEvent<HTMLAnchorElement>) => {
    if (paymentAttemptUnsettled) event.preventDefault();
    else
      storeCheckoutHandoff(
  ...
  if (loadState === 'unavailable') {
    return (
      <section className={styles.page}>
        ...
        <CheckoutPayment canPay={false} draftId={null} />
      </section>
    );
  }
  ...
  return (
    <section
      aria-labelledby={
        paymentSucceeded ? 'payment-success-title' : 'checkout-title'
      }
    ...
    >
      ...
      <form
        className={paymentSucceeded ? styles.successLayout : styles.layout}
        onSubmit={save}
      >
        {paymentSucceeded ? null : (
            ...
            >
              <div className={styles.fields}>
                ...
                <div className={styles.notes}>
                  <label htmlFor="checkout-additional-info">
                    Delivery notes
                  </label>
                  <textarea
...
function normalizeProfileCountry(country: string | null | undefined) {
  ...
  const alias = PROFILE_COUNTRY_CODES[normalized.toLocaleLowerCase('en-US')];
  if (alias) return alias;
  const countryCode = normalized.toUpperCase();
  return ISO_ALPHA_2_COUNTRY_CODES.has(countryCode) ? countryCode : '';
}
