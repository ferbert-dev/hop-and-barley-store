...
3. Read Notion `self.current_tool_access` and require read plus create/update capability for pages, databases, data sources, and page properties before the first write. Confirm GitHub read access separately.
4. Ask before any external creation if the user has not explicitly requested creation.
5. Fetch the intended Notion parent and schema before creation. If any required capability, relation target, property, or status option is unavailable, stop with zero writes.
...
3. If no exact match exists, create a new Project Hub and four new databases under it: Architecture, Tickets, Agent Registry, and Agent Runs. Create relations exactly as specified in `references/notion-schema.md`. After each checkpoint, read it back before continuing.
...
5. Run `python3 scripts/init_local.py <project-root> --project-name "<name>" --mode core`. Review the reported files. Do not overwrite existing files unless the user explicitly approves it.
...
8. Run `python3 scripts/verify_local.py <project-root>` and read back the live Notion schema. Report byte drift, missing relations, invalid status options, or unavailable connectors as blockers.

If a read-back fails after an external checkpoint, stop immediately and record the exact created IDs as an incomplete installation. Resume only by repairing that exact name-plus-repository installation; never create a duplicate. Do not delete partial Notion state automatically. Offer user-authorized repair or removal after showing the affected IDs.
...
- The portable installer exposes only `core` mode. A future mode needs distinct assets, verification, and a separate ticket before it is advertised.
